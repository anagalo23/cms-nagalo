...
<body">
 ...
 %> <%--affichage du message --%> date de publication:<br />
	...
	<%= RedacteurDAO.getInstance().unRedacteur(m.getIdRedacteur()).getNom().toUpperCase()%>
	 ...
	 <%=RedacteurDAO.getInstance().unRedacteur(m.getIdRedacteur()).getMail() %><br/>
	</p>
	<hr />
	<br />
	<br />
	...
	%> pas de commentaire. Commenter plus bas<%
		} else {%> <%for (CommentaireDTO c : com) {%>


	<p>

		pseudo: <i><%=StringEscapeUtils.escapeHtml(c.getPseudo())%></i><br />
		Date du commentaire:<br/><%=c.getDate().substring(0,10)%><br /> <%=c.getDate().substring(11,19)%><br/>Contenu du commentaire:<b>
